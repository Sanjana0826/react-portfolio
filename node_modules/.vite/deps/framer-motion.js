...
function getWillChangeName(name) {
  if (transformProps.has(name)) {
    return "transform";
  } else if (acceleratedValues.has(name) || // Manually check for backgroundColor as accelerated animations
...
function updateMotionValuesFromProps(element, next, prev) {
  for (const key in next) {
    ...
    if (isMotionValue(nextValue)) {
      ...
      if (true) {
        warnOnce(nextValue.version === "11.3.1", `Attempting to mix Framer Motion versions ${nextValue.version} with 11.3.1 may not work as expected.`);
      }
